...
const WareHouseOwnerMaster = () => {
  ...
  const viewForm = (info) => {
    ...
    const ownerId = info.row.original.id;

    console.log("Navigating to role id:", ownerId);

    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_WAREHOUSE_WAREHOUSE_OWNER_EDIT}/${ownerId}`, {
      state: {
  ...
  const editForm = (info) => {
    ...
    navigate(
      `${ROUTE_PATH.ALL_MASTER_MANAGE_WAREHOUSE_WAREHOUSE_OWNER_EDIT}/${editedFormId}`,
      {
  ...
  const addForm = () => {
    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_WAREHOUSE_WAREHOUSE_OWNER_ADD}`);
  };
