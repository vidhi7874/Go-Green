...
function BankBranchMaster() {
  ...
  const viewForm = (info) => {
    ...
    const bankBranchId = info.row.original.id;

    console.log("Navigating to role id:", bankBranchId);

    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_BANKS_BANK_BRANCH_EDIT}/${bankBranchId}`, {
      state: {
  ...
  const addForm = () => {
    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_BANKS_BANK_BRANCH_ADD}`);
  };
  ...
  const editForm = (info) => {
    ...
    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_BANKS_BANK_BRANCH_EDIT}/${editedFormId}`, {
      state: { details: info.row.original },
    });
  };
  // Edit Form Link Function End

  // BankBranch Master Get Api Start
