...
const SecurityAgencyMaster = () => {
  ...
  const [filter, setFilter] = useState({
    ...
    total: 0,
    add: false,
    modal: "SecurityAgency",
    excelDownload: "SecurityAgency",
  });
  ...
  const viewForm = (info) => {
    ...
    const agencyId = info.row.original.id;

    console.log("Navigating to role id:", agencyId);

    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_VENDORS_SECURITY_AGENCY_EDIT}/${agencyId}`, {
      state: {
  ...
  const editForm = (info) => {
    ...
    navigate(
      `${ROUTE_PATH.ALL_MASTER_MANAGE_VENDORS_SECURITY_AGENCY_EDIT}/${editedFormId}`,
      {
  ...
  const addForm = () => {
    navigate(`${ROUTE_PATH.ALL_MASTER_MANAGE_VENDORS_SECURITY_AGENCY_ADD}`);
  };
