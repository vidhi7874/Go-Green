...
const WarehouseInspection = React.memo(() => {
  ...
  const warehouseOwnerOnEdit = (item, index) => {
    ...
    warehouseOwnerFormsMethod.setValue("id", item?.id, {
      shouldValidate: false,
    });
  ...
  useEffect(() => {
    ...
    oilTankDetailsFormsMethod.setValue(
      ...
      (
        ...
        (Number(oilTankDetailsFormsMethod.getValues("diameter") || 0) / 2) *
        (Number(oilTankDetailsFormsMethod.getValues("diameter") || 0) / 2)
      ).toFixed(2),
      {
        shouldValidate: true,
  ...
  useEffect(() => {
    siloDetailsFormsMethod.setValue(
      ...
      (
        ...
        (Number(siloDetailsFormsMethod.getValues("diameter") || 0) / 2) *
        (Number(siloDetailsFormsMethod.getValues("diameter") || 0) / 2)
      ).toFixed(2),
      {
        shouldValidate: true,
  ...
  const saveAsDraftFunction = async () => {
    ...
    removeLastUpdatedUser(finalData);
    console.log("first", finalData);

    // Function to create a new object without a specified property
    function removeProperty(obj, propToDelete) {
      const newObj = { ...obj }; // Create a shallow copy of the object

      if (newObj.hasOwnProperty(propToDelete)) {
        delete newObj[propToDelete]; // Delete the property if it exists
      }

      for (const key in newObj) {
        if (typeof newObj[key] === "object" && newObj[key] !== null) {
          newObj[key] = removeProperty(newObj[key], propToDelete);
        }
      }
    ...
    // Iterate through the array and replace each object without 'last_updated_user' property
    const newArray = finalData.client.map((obj) =>
      removeProperty(obj, "last_updated_user")
    );

    // Replace the original array with the modified one
    finalData.client = newArray;

    // Now, finaltdata.client has the 'last_updated_user' property removed from each object
  ...
  const assignToMeFunction = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // }, 2000);
        // navigate(`${ROUTE_PATH.WAREHOUSE_INSPECTION_MASTER}`);
      }
    } catch (error) {
      console.error("Error:", error);
      let errorMessage =
      ...
      if (item.key === "region") {
        ...
        if (item?.index?.length > 0) {
          ...
          const tempChamber =
            item?.index?.map((old2) => ({
              ...old2,
              warehouse_chamber_id: old2?.warehouse_chamber_id?.id || null,
            })) || [];
          if (data?.warehouse_unit_type?.warehouse_unit_type === "Oil Tank") {
            setOilTankDetails(tempChamber);
          } else if (
            data?.warehouse_unit_type?.warehouse_unit_type === "Silo"
          ) {
            setSiloDetails(tempChamber);
          } else {
            setChamberDetails(tempChamber);
          }
        }
        ...
        if (item?.index?.length > 0) {
          ...
          setWarehouseOwnersDetails(tempOwner);
        }
  ...
  useMemo(() => {
    setError("warehouse_unusable_area", {
      message:
        Number(getValues("warehouse_total_area") || 0) >
        Number(getValues("warehouse_unusable_area") || 0)
          ? ""
  ...
  const onSubmit = async (data) => {
    ...
    if (
      (data?.warehouse_unit_type == 10 ||
        data?.warehouse_unit_type == 9 ||
        data?.warehouse_unit_type == 6) &&
      clientsDetails?.length === 0
    ) {
      showToastByStatusCode(400, "Silo details is required");
      return false;
    }

    if (
      (data?.warehouse_unit_type == 7 || data?.warehouse_unit_type == 8) &&
    ...
    const breadcrumbArray = [
      {
        ...
        link: `${ROUTE_PATH.WAREHOUSE_INSPECTION_MASTER}`,
      },
    ];
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <Box>
                          ...
                          >
                            ...
                            <GridItem colSpan={{ base: 1 }}>
                              ...
                              >
                                {console.log(errors, "errorsteseting")}
                                <Input
                                  type="number"
                                  {...register(
                                    schema_obj.first_accordion
            ...
            {Number(saveData?.status?.status_code || 0) > 0 ? (
              <Box bg="white" borderRadius="10px" p="20px" mt="20px">
                ...
                Number(saveData?.status?.status_code || 0) === 5 ||
                Number(saveData?.status?.status_code || 0) === 8 ||
                Number(saveData?.status?.status_code || 0) === 10 ||
                Number(saveData?.status?.status_code || 0) === 7 ||
                Number(saveData?.status?.status_code || 0) === 4 ? (
                  <Grid
                    textAlign="right"
                    alignItems="center"
                    my="3"
