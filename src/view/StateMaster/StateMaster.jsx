...
const StateMaster = () => {
  ...
  const viewForm = (info) => {
    ...
    const stateId = info.row.original.id;

    console.log("Navigating to role id:", stateId);

    navigation(`${ROUTE_PATH.ALL_MASTER_MANAGE_LOCATION_STATE_EDIT}/${stateId}`, {
      state: {
  ...
  const addForm = () => {
    // navigation(`all-master/manage-location/add/state-master`);
    navigation(
      `${ROUTE_PATH.ALL_MASTER_MANAGE_LOCATION_STATE_ADD}`
    );
  ...
  const editForm = (info) => {
    navigation(
      ...
      `${ROUTE_PATH.ALL_MASTER_MANAGE_LOCATION_STATE_EDIT}/${info.row.original.id}`, {
