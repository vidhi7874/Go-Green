...
function AddEditSupervisorHiringMaster() {
  ...
  const addData = async (data) => {
    try {
      const response = await addUserMaster({
        ...data,
        user_location: [
          {
            area: details.area,
  ...
  //     setSelectBoxOptions((prev) => ({
  //       ...prev,
  //       chamber: response?.map(({ chamber_number, id, warehouse }) => ({
    ...
    const breadcrumbArray = [
      {
        ...
        link: `${ROUTE_PATH.ALL_MASTER_MANAGE_USERS_SUPERVISOR_HIRING}`,
      },
    ];
